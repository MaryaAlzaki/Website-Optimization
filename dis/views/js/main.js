...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    var i=0;
var randomePizza=document.getElementsByClassName("randomPizzaContainer");
var dx = determineDx(randomePizza[0], size);
      console.log("Hello"+dx);

      var newwidth = (randomePizza[i].offsetWidth + dx) + 'px';

    for (i = 0; i < randomePizza.length; i++) {

      randomePizza[i].style.width = newwidth;
    }
  }
...
var counter=0;
  ...
  for (var j = 0; j < 5; j++){
   ...
   phase[j] = Math.sin((scrollTop / 1250) + (j % 5));

    console.log(phase[j], document.body.scrollTop/1250);
  }
...
if(counter==5){
    ...
    items[i].style.left = items[i].basicLeft + 100 * phase[counter] + 'px';
